      ...
      <Dialog open={open} onOpenChange={handleClose}>
        <DialogContent className="max-h-[90vh] w-[min(100vw-2rem,56rem)] max-w-none gap-0 overflow-hidden border border-slate-700 bg-black p-0 text-white sm:rounded-xl">
          <DialogHeader className="sr-only">
            ...
            {activeEntry && (
              <DialogDescription>
                Short walkthrough of the {activeEntry.title} experience in Constructify.
              </DialogDescription>
            )}
          </DialogHeader>
          {activeEntry && (
            <>
              <div className="relative aspect-video w-full bg-black">
                <iframe
                  key={activeEntry.youtubeId}
                  className="absolute inset-0 h-full w-full border-0"
                  src={`https://www.youtube.com/embed/${activeEntry.youtubeId}?rel=0&autoplay=1`}
                  title={`${activeEntry.title} — Constructify demo`}
